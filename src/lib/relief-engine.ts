...
export function buildGeometry(hg: HeightGrid, s: ReliefSettings): GeometryResult {
  ...
  if (s.join && hasL) notches.push({ x0: -1, y0: H / 2 + oy - (tw + 2 * clr) / 2, x1: to + clr, y1: H / 2 + oy + (tw + 2 * clr) / 2 });
  if (s.join && hasD) notches.push({ x0: W / 2 + ox - (tw + 2 * clr) / 2, y0: -1, x1: W / 2 + ox + (tw + 2 * clr) / 2, y1: to + clr });
  ...
  return {
    ...
    nx,
    ny,
    bbox: [hasR ? W + to : W, hasU ? H + to : H, base + relief],
  };
}
